...
//    }
    ...
    bool read = false;
    sscanf(line," %s",str);
    // check that 6th word is Edges
    if(0==strcmp(str,"Edges"))
    {
        int number_of_edges;
        ...
        for (int i = 0; i < number_of_edges; i++) {
            if (3 != fscanf(mesh_file, " %d %d %d ", &edge[0], &edge[1], &extra)) {
                printf("Error: expecting edge indices...\n");
                return false;
            }
            for (int j = 0; j < 2; j++) {
                E(i, j) = edge[j] - 1;
    ...
    if(read)
    {
        sscanf(line," %s",str); //scan new line
    }

    read = false;
    // check that 7th word is Triangles
    if(0==strcmp(str,"Triangles"))
    {
        int number_of_triangles;
        if(1 != fscanf(mesh_file," %d",&number_of_triangles))
    ...
    // check that 8th word is Tetrahedra
    if(0!=strcmp(str,"Tetrahedra"))
    {
        fprintf(stderr,"Error: 8th word should be Tetrahedra not %s\n",str);
        fclose(mesh_file);
        return false;
    }
    int number_of_tetrahedra;
